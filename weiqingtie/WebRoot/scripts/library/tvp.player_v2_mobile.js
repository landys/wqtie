...
function empty(){};$.ajaxJSONP=function(options){if(!('type'in options))return $.ajax(options);var callbackName='jsonp'+(++jsonpID),script=document.createElement('script'),cleanup=function(){clearTimeout(abortTimeout)
$(script).remove()
...
el[(el.css('opacity')==0||el.css('display')=='none')?'fadeIn':'fadeOut'](speed,callback)})}})(Zepto);tvp.$={};(function(arr){for(var i=0,len=arr.length;i<len;i++){if(typeof window[arr[i]]=="function"){tvp.$=window[arr[i]];break;}}})(["jQuery","jq","Zepto"]);;(function($){function detect(ua){var MQQBrowser=ua.match(/MQQBrowser\/(\d+\.\d+)/i),WeChat=ua.match(/MicroMessenger\/((\d+)\.(\d+))\.(\d+)/),MacOS=ua.match(/Mac\sOS\sX\s(\d+\.\d+)/),WinOS=ua.match(/Windows(\s+\w+)?\s+?(\d+\.\d+)/),MiuiBrowser=ua.match(/MiuiBrowser\/(\d+\.\d+)/i),UC=ua.match(/UCBrowser\/(\d+\.\d+(\.\d+\.\d+)?)/)||ua.match(/\sUC\s/),IEMobile=ua.match(/IEMobile(\/|\s+)(\d+\.\d+)/),ipod=ua.match(/(ipod\sOS)\s([\d_]+)/);if(window.ActiveXObject){var vie=6;(window.XMLHttpRequest||(ua.indexOf('MSIE 7.0')>-1))&&(vie=7);(window.XDomainRequest||(ua.indexOf('Trident/4.0')>-1))&&(vie=8);(ua.indexOf('Trident/5.0')>-1)&&(vie=9);(ua.indexOf('Trident/6.0')>-1)&&(vie=10);this.browser.ie=true,this.browser.version=vie;}
...
return guid;},formatSize:function(size){var s=""+size;if(s.indexOf("%")>0)return s;if(s.indexOf("px")>0)return s;if(/^\d+$/.test(s))return s+"px";return s;}}
...
swipeTimeout=setTimeout(function(){touch.el.trigger('swipe');touch.el.trigger('swipe'+(swipeDirection(touch.x1,touch.x2,touch.y1,touch.y2)));touch={}},0);else if('last'in touch)
tapTimeout=setTimeout(function(){var event=$.Event('tap');event.cancelTouch=cancelAll;touch.el.trigger(event);if(touch.isDoubleTap){touch.el.trigger('doubleTap');touch={};}
...
return false;},isEnforceMP4:function(){var ua=navigator.userAgent,m=null;if(!!tvp.$.os.android){if(tvp.$.browser.firefox){return true;}
...
vflash=ag.join(".");return vflash;},getFlashMain:function(){return parseInt(tvp.version.getFlash().split(".")[0],10);}}})();tvp.VAL_DEFINE={AUTO:-1,TRUE:1,FALSE:0};tvp.PLAYER_DEFINE={LIVE:1,VOD:2};tvp.defaultConfig={video:null,width:600,height:450,autoplay:true,mute:false,volume:50,modId:"mod_player",playerid:"",coverId:"",typeId:0,pic:"",type:tvp.PLAYER_DEFINE.VOD,playerType:"auto",loadingSwf:"",isPay:false,isHtml5AutoBuffer:false,isHtml5UseAirPlay:true,isHtml5ControlAlwaysShow:false,html5Preload:"null",html5VodUIFeature:['controlbar','tips','playpause','progress','timepanel','definition','fullscreen',"overlay","bigben","posterlayer","shadow"],html5LiveUIFeature:['controlbar','tips','playpause','fullscreen',"overlay","posterlayer","shadow"],html5FeatureExtJS:{},html5ForbiddenUIFeature:[],isHTML5UseUI:false,isHTML5ShowPosterOnStart:true,isHTML5ShowPosterOnEnd:false,isHTML5ShowPosterOnChange:true,isiPhoneShowPosterOnPause:true,isHtml5ShowPlayBtnOnPause:true,isHtml5UseFakeFullScreen:true,flashWmode:"window",vodFlashUrl:"",vodFlashType:"TPout",vodFlashLoadingSwf:"",vodFlashExtVars:{},vodFlashListType:2,vodFlashSkin:"",isVodFlashShowCfg:true,isVodFlashShowEnd:true,isVodFlashShowSearchBar:true,isVodFlashShowShare:true,isVodFlashShowNextBtn:true,liveFlashUrl:"",liveFlashSwfType:"TencentPlayerLive",liveFlashLoadingSwf:"",isLiveFlashShowConfigBtn:true,isLiveflashShowFullBtn:true,isLiveFlashShowShareBtn:true,isLiveFlashShowCfg:true,ocxControlBar:"",ocxLoadingSwf:"",ocxControlHeight:60,ocxSkin:"",isOcxShowPauseBtn:false,isOcxHideControl:false};;(function($){var tmpl=(function(cache,$){return function(str,data){var fn=!/\s/.test(str)?cache[str]=cache[str]||tmpl(document.getElementById(str).innerHTML):function(data){var i,variable=[$],value=[[]];for(i in data){variable.push(i);value.push(data[i]);};return(new Function(variable,fn.$)).apply(data,value).join("");};fn.$=fn.$||$+".push('"+str.replace(/\\/g,"\\\\").replace(/[\r\t\n]/g," ").split("<%").join("\t").replace(/((^|%>)[^\t]*)'/g,"$1\r").replace(/\t=(.*?)%>/g,"',$1,'").split("\t").join("');").split("%>").join($+".push('").split("\r").join("\\'")+"');return "+$;return data?fn(data):fn;}})({},'$'+(+new Date));$.tmpl=tmpl;})(tvp.$);tvp.report=(function(){var isFree=true;var reportObj=null;var urlList=[];function errorHandle(){if(urlList.length==0){isFree=true;reportObj=null;return;}
...
this.setCurVideo(this.config.video);this.write(this.config.modId);},write:function(id){$("#"+id).html("here is player of base");},log:function(msg){if(window.console){window.console.log(msg);}},callCBEvent:function(eventName){var fn=null;if($.isFunction(this[eventName])&&this[eventName]!=tvp.$.noop){fn=this[eventName];}else if(this.instance&&$.isFunction(this.instance[eventName])&&this.instance[eventName]!=tvp.$.noop){fn=this.instance[eventName];}
if($.isFunction(fn)){var args=Array.prototype.slice.call(arguments,1);return fn.apply(this,args);}
return undefined;}}})(tvp,tvp.$);;(function(tvp,$){tvp.BaseHtml5=function(){this.protectedFn={},this.h5EvtAdapter={},this.eventList=this.eventList.concat(["html5error"]),this.html5AttrList={"autobuffer":"isHtml5AutoBuffer","preload":"html5Preload","autoplay":"autoplay","x-webkit-airplay":"isHtml5UseAirPlay"};this.$videomod=null;};tvp.BaseHtml5.fn=tvp.BaseHtml5.prototype=new tvp.BasePlayer();$.extend(tvp.BaseHtml5.fn,{getCurVid:function(){if(this.curVid=="")return(this.curVideo instanceof tvp.VideoInfo)?this.curVideo.getVid():"";return this.curVid;},getPlayer:function(){return this.videoTag;},getPlayerType:function(){return"html5";},createVideoHtml:function(){this.playerid=this.config.playerid;if(!this.playerid){this.playerid="tenvideo_video_player_"+(tvp.BaseHtml5.maxId++);}
...
defer.resolve(json);}).fail(function(){defer.reject(500);});return defer;}}})(tvp,tvp.$);;(function(tvp,$){var _isInited=false;function Html5TinyPlayer(vWidth,vHeight){var $me=this,h5EvtAdapter={};this.videoTag=null,this.$video=null,this.config.width=tvp.$.filterXSS(vWidth),this.config.height=tvp.$.filterXSS(vHeight),this.protectedFn={},this.isUseControl=true,$.extend(this.h5EvtAdapter,{"onEnded":function(){this.onended();var nextVid="",vidArr=this.curVideo.getVidList().split("|"),vidIndexOf=$.inArray(this.curVid,vidArr);if(vidIndexOf<vidArr.length-1){nextVid=vidArr[vidIndexOf+1];}
...
var nextVideoInfo=this.callCBEvent("ongetnext",this.curVid,this.curVideo);if(!!nextVideoInfo&&nextVideoInfo instanceof tvp.VideoInfo){this.play(nextVideoInfo);}},"onError":function(ts,e){var errContent=-1;if(!!e.target&&e.target.error){errContent=e.target.error.code;}
if(errContent!=4){return;}
this.showError(0,errContent);},"onPlaying":function(){this.onplaying(this.curVid,this.curVideo);}});};Html5TinyPlayer.fn=Html5TinyPlayer.prototype=new tvp.BaseHtml5();$.extend(Html5TinyPlayer.prototype,{registerPlugins:function(){var t=this,authorityPluginsList=["monitor"];$.each(authorityPluginsList,function(i,v){try{var evtName="build"+v;if($.isFunction(t[evtName])){t[evtName](t);}}catch(err){tvp.debug("[registerPlugins]:"+err.message);}});},write:function(modId){tvp.BaseHtml5.prototype.write.call(this,modId);this.registerPlugins();this.callProtectFn("onwrite");this.onwrite();this.oninited();this.play(this.curVideo,this.config.autoplay);}});$.extend(Html5TinyPlayer.prototype,{pause:function(){this.videoTag.pause();},play:function(v,isAutoPlay){var t=this,isVidChange=false;if($.isUndefined(isAutoPlay))isAutoPlay=true;if($.isUndefined(v)){t.videoTag.play();return;}
if(v instanceof tvp.VideoInfo){isVidChange=(v.getVid()!=t.curVid&&t.curVid!="");t.setCurVideo(v);if(isVidChange){t.callCBEvent("onchange",t.curVideo.getFullVid());if($.os.iphone){t.videoTag.pause();t.videoTag.play();}}
v.setPid($.createGUID());t.curVid=t.curVideo.getFullVid();}
...
t.$video.trigger("tvp:video:ajaxerror");t.$video.trigger("tvp:video:error",errcode,errcontent);t.showError(errcode,errcontent);}).always(function(){t.curVid=t.curVideo.lastQueryVid;});},seek:function(time){if(isNaN(time))return;this.videoTag.currentTime=time;},getCurTime:function(){return this.videoTag.currentTime;},checkIsPlayingLoop:function(times){times=times||0;var t=this;if(!!this.playinglooptimer)clearTimeout(this.playinglooptimer);if(this.videoTag.currentTime===0&&times<=30){this.videoTag.load();this.videoTag.play();this.playinglooptimer=setTimeout(function(){t.checkIsPlayingLoop(++times);},1000);}},setPoster:function(){var poster=this.curVideo.getPoster();if($.isString(poster)&&poster.length>0){this.videoTag.poster=poster}else{this.hidePoster();}},hidePoster:function(){this.videoTag.removeAttribute("poster");},getDuration:function(){var dur=this.curVideo.getDuration();if(!isNaN(dur)&&dur>0){return dur}
return this.videoTag.duration;}});tvp.Html5Tiny=Html5TinyPlayer;})(tvp,tvp.$);;(function(tvp,$){function Html5LiveTiny(vWidth,vHeight){this.config.width=tvp.$.filterXSS(vWidth),this.config.height=tvp.$.filterXSS(vHeight),this.videoTag=null,this.$video=null,this.protectedFn={},this.isUseControl=true;$.extend(this.h5EvtAdapter,{"onError":function(ts,e){var errContent=-1;if(!!e.target&&e.target.error){errContent=e.target.error.code;}
this.callCBEvent("onhtml5error",0,errContent);}});}
Html5LiveTiny.fn=Html5LiveTiny.prototype=new tvp.BaseHtml5();$.extend(Html5LiveTiny.fn,{write:function(id){tvp.BaseHtml5.prototype.write.call(this,id);this.callProtectFn("onwrite");this.callCBEvent("onwrite");this.callCBEvent("oninited");this.play(this.curVideo,this.config.autoplay);},play:function(video,isAutoPlay){if(!this.videoTag){throw new Error("未找到视频播放器对象，请确认<video>标签是否存在");}
if(!video instanceof tvp.VideoInfo){throw new Error("传入的对象不是tvp.VideoInfo的实例");}
if($.isUndefined(isAutoPlay))isAutoPlay=true;this.setCurVideo(video);var cnlId=video.getChannelId();var guid=tvp.$.createGUID();var new_url="http://zb.v.qq.com:1863/?progid="+cnlId+"&ostype=ios"+"&rid="+encodeURIComponent(guid);this.videoTag.src=new_url;this.$video.trigger("tvp:video:src");this.videoTag.pause();if(isAutoPlay){this.videoTag.load();this.videoTag.play();}
...
this.modId=id;this.$mod=$("#"+id);this.oninited();var htmlBuf=tvp.MP4Skin.html;videoModId="mod_"+this.playerid;var $videomod=$('<div id="'+videoModId+'"></div>').appendTo(t.$mod);this.$elements=$(htmlBuf).appendTo($videomod).css("width",t.config.width).css("height",t.config.height);this.videomod=$.getByID(videoModId);this.callCBEvent("onwrite");this.play(this.curVideo);},play:function(v){var t=this;if(v instanceof tvp.VideoInfo){isVidChange=(v.getVid()!=t.curVid&&t.curVid!="");t.setCurVideo(v);if(isVidChange){t.onchange(t.curVideo.getFullVid());}
t.curVid=t.curVideo.getFullVid();}
t.curVideo.getMP4Url().done(function(url){var $link=t.$elements.find("a");$link.attr("href",url);t.onplay(t.curVideo.lastQueryVid,t.curVideo);}).fail(function(errCode,errContent){t.showError(errCode,errContent);t.onerror(errCode,errContent);}).always(function(){t.curVid=t.curVideo.lastQueryVid;});},getPlayerType:function(){return"mp4";}});tvp.MP4Link.maxId=0;})(tvp,tvp.$);;(function(tvp,$){tvp.BaseFlash=function(){var $me=this;this.swfPathRoot="http://imgcache.qq.com/tencentvideo_v1/player/";this.flashobj=null;this.flashVarsKeyMapToCfg={};}
...
if(!el)return;var str=this.getFlashHTML(),videoModId="mod_"+this.playerid;el.innerHTML='<div id="'+videoModId+'">'+str+'</div>';this.flashobj=$.browser.ie?document.getElementById(this.playerid):document.embeds[this.playerid];this.videomod=$.getByID(videoModId);this.callCBEvent("onwrite");},getPlayer:function(){return this.flashobj;}})})(tvp,tvp.$);;(function(tvp,$){tvp.FlashLivePlayer=function(vWidth,vHeight){var $me=this;tvp.BaseFlash.maxId++;this.flashVarsKeyMapToCfg={"showcfg":"isLiveFlashShowCfg","loadingswf":"liveFlashLoadingSwf","share":"isLiveFlashShowShareBtn","pic":"pic","autoplay":"autoplay","ispay":"isPay"};this.swfPathRoot="http://imgcache.qq.com/minivideo_v1/vd/res/";this.config.width=$.filterXSS(vWidth);this.config.height=$.filterXSS(vHeight);this.loginResponse=function(){if(!this.flashobj||typeof this.flashobj.loginCallback=="function"){this.flashobj.loginCallback(tvp.FlashLivePlayer.flashloginParam);tvp.FlashLivePlayer.flashloginParam={};}}
window.playerInit=function(){$me.callCBEvent("oninited");if($me.config["autoplay"]==1){$me.play($me.curVideo);}}}
tvp.FlashLivePlayer.prototype=new tvp.BaseFlash();$.extend(tvp.FlashLivePlayer.prototype,{getChannelURl:function(cnlid){return"http://zb.v.qq.com:1863/?progid="+cnlid;},getFlashVar:function(){var flashvar='',funPrefix="TenVideo_FlashLive_",varsVal=this.getFlashVarVal(),linkChar="";if(!!this.config["autoplay"]){flashvar+="vid="+this.curVideo.getChannelId();flashvar+="&vurl="+this.getChannelURl(this.curVideo.getChannelId());flashvar+="&sktype="+(!!this.curVideo.getIsLookBack()?"vod":"live");linkChar="&";}
flashvar+=linkChar;flashvar+="funCnlInfo="+funPrefix+"GetChannelInfo"
flashvar+="&funTopUrl="+funPrefix+"GetTopUrl";flashvar+="&funLogin="+funPrefix+"IsLogin";flashvar+="&funOpenLogin="+funPrefix+"OpenLogin";flashvar+="&funSwitchPlayer="+funPrefix+"SwitchPlayer";$.each(varsVal,function(k,v){if(($.isString(v)&&v.length>0)||$.type(v)=="number"){flashvar+="&"+k+"="+$.filterXSS(v);}});return flashvar;},play:function(video){if(!this.flashobj){return;}
...
window.TenVideo_FlashLive_SwitchPlayer=$.noop;})(tvp,tvp.$);var preplay=tvp.$.noop,nextplay=tvp.$.noop,attrationstop=tvp.$.noop,thisplay=tvp.$.noop,playerInit=tvp.$.noop;;(function(tvp,$){tvp.FlashPlayer=function(vWidth,vHeight){var $me=this,pauseTime=-1;this.flashVarsKeyMapToCfg={"cid":"coverId","tpid":"typeId","showend":"isVodFlashShowEnd","showcfg":"isVodFlashShowCfg","searchbar":"isVodFlashShowSearchBar","loadingswf":"vodFlashLoadingSwf","share":"isVodFlashShowShare","pic":"pic","skin":"vodFlashSkin","shownext":"isVodFlashShowNextBtn","list":"vodFlashListType","autoplay":"autoplay","ispay":"isPay"};tvp.BaseFlash.maxId++;this.isStartPlay=false;this.getPlayerType=function(){return"flash";}
...
if(!video instanceof tvp.VideoInfo){throw new Error("传入的对象不是tvp.VideoInfo的实例");}
var isVideChange=this.curVid!=video.getFullVid();this.setCurVideo(video);if(isVideChange){this.callCBEvent("onchange",this.curVideo.getFullVid());}
this.curVid=this.curVideo.getFullVid();this.isStartPlay=false;var vstart=0,vend=0,tagstart=0,tagend=0;if(video.getIdx()==0){vstart=video.getPrefix()||0;vend=video.getEndOffset()||0;}else{tagstart=video.getTagStart();tagend=video.getTagEnd();}
...
tvp.$.each(varsVal,function(k,v){if(($.isString(v)&&v.length>0)||$.type(v)=="number"){flashvar+="&"+k+"="+tvp.$.filterXSS(v);}});if(this.curVideo.getPay()>0){flashvar+="&pay="+tvp.$.filterXSS(this.curVideo.getPay());}
...
for(var p in this.config.vodFlashExtVars){flashvar+=["&",tvp.$.filterXSS(p),"=",encodeURIComponent(this.config.vodFlashExtVars[p])].join("");}
...
return-1;},showPlayer:function(){if(!this.flashobj)return;var width=""+this.config.width,height=""+this.config.height;if(width.indexOf("px")<0){width=parseInt(width)+"px";}
...
var create=function(config){var defer=$.Deferred();$.extend($.extend(cfg,tvp.defaultConfig),config);if(!cfg.video instanceof tvp.VideoInfo){throw new Error("video未初始化");return;}
$.when(cfg.type==tvp.PLAYER_DEFINE.VOD?checkVodPlayer():checkLivePlayer(cfg.video)).then(function(){if(!playerClass){throw new Error("您当前使用的统一播放器JS文件不包含指定的播放器内核");return;}
var t=new playerClass();t.init(cfg);defer.resolve(t);});return defer;};tvp.create=create;var oldParamMap={"showcfg":["isVodFlashShowCfg","isLiveFlashShowCfg"],"searchbar":["isVodFlashShowSearchBar"],"showend":["isVodFlashShowEnd"],"tpid":["typeId"],"cid":["coverId"],"flashshownext":["isVodFlashShowNextBtn"],"loadingswf":"loadingSwf","wmode":"flashWmode","flashskin":["vodFlashSkin"],"extvars":["vodFlashExtVars"],"swftype":["vodFlashType"],"swfurl":["vodFlashUrl","liveFlashUrl"]};tvp.Player=function(vWidth,vHeight){this.instance=null,this.config={},this._oldcfg={};$.extend(this.config,tvp.defaultConfig);this.setting("width",vWidth);this.setting("height",vHeight);};tvp.Player.prototype=new tvp.BasePlayer();tvp.Player.prototype={setting:function(k,v){this.config[k]=v;},output:function(id){this.setting("modId",id);this.create(this.config);},create:function(config){var t=this;tvp.create(config).done(function(f){t.instance=f;t.instance.instance=t;for(var p in t.instance){if(p=="instance")continue;if(p.substr(0,2)=="on"&&$.isFunction(t[p])&&t[p]!=tvp.$.noop)continue;t[p]=t.instance[p];}
if(config.type==tvp.PLAYER_DEFINE.LIVE){t.play=function(v){if($.isString(v)){t.config.video.setChannelId(v);v=t.config.video;}else if(v instanceof tvp.VideoInfo){$.when(checkLivePlayer(v)).then(function(){if(t.instance instanceof playerClass){t.instance.play(v);}else{config.video=v;create(config);}});}}}});},addParam:function(k,v){if(k=="config"&&$.type(v)=="object"){$.extend(this.config,v);}else{this._oldcfg[k]=v;}},setCurVideo:function(v){this.config["video"]=v;},write:function(id){this.config.modId=id;var type=this._oldcfg["type"]==1?1:2,t=this;$.each(this._oldcfg,function(k,v){if(k in oldParamMap){if($.isArray(oldParamMap[k])){if(type==2){t.config[oldParamMap[k][0]]=v;}else if(type==1&&oldParamMap[k].length>=2){t.config[oldParamMap[k][1]]=v;}}else if($.isString(oldParamMap[k])){t.config[oldParamMap[k]]=v;}}else if(k in tvp.defaultConfig){t.config[k]=v;}});delete this._oldcfg;this.create(this.config);}}})(tvp,tvp.$);tvp.Player.instance=[];/*  |xGv00|e8b122d416f97405dee91b5c7d8f01e2 */