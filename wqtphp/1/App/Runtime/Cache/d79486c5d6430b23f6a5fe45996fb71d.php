...
<body title="<?php echo ($card["title"]); ?>" icon="__IMAGE_PUBLIC__/<?php echo ($card["coverPhotoUrl"]); ?>"
	link="<?php echo ($pageUrl); ?>"
	...
	<div class="t" data-role="page">
			...
			<div class="t_content">
				<div class="t_top">
					...
					<audio id="music" autoplay="autoplay" loop> <source
						src="__IMAGE_PUBLIC__/<?php echo ($card["musicUrl"]); ?>" id="music_url_mp3"
				...
				<div class="t_bg" style="background:url(__IMAGE_PUBLIC__/<?php echo ($t2Bg); ?>) no-repeat;">
					<ul class="t_nav_a">
				...
				<div id="divPhotos" class="div_border" style="display: none">
					<?php if($nPhotos > 0): ?><ul>
						<?php if(is_array($photos)): $i = 0; $__LIST__ = $photos;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$photo): $mod = ($i % 2 );++$i; if($photo["url"] != null): ?><li><img src="__IMAGE_PUBLIC__/<?php echo ($photo["url"]); ?>" width="100%"></li><?php endif; endforeach; endif; else: echo "" ;endif; ?>
					</ul><?php endif; ?>
				</div>
		...
		<div class="t_foot" data-role="foot">
			...
			<?php if($card["agentQcodePath"] != null): ?><a
				...
				style="width: 80px;" src="__IMAGE_PUBLIC__/<?php echo ($card["agentQcodePath"]); ?>"></a><?php endif; ?>
		</div>
	</div>
</body>
